...
//* the presentation of the existing data.
<script>
  ...
  export default {
    ...
    computed: {
      addClasses() {
        ...
        console.log("id user", nowId);
        let responce = this.results;
        console.log("this is outcome in computed - ", responce);
        for (let item of responce) {
          console.log("arry here-", item);
          console.log("arry statues-", item.statuses);
          for (let i of item.statuses) {
            console.log(i.stat_postId);
            //* if poststatus id == post id && userid = userstaus id
            if (nowId === i.stat_userId && item.post_id === i.stat_postId) {
              console.log("yes -", item.post_id);
